...
Have same phase sequence as original vectors
Displaced by 120degrees

**Negative sequence:**

Have opposite phase sequence as original vectors
Displaced by 120degrees

**Zero sequence:**

All vectors are in-phase with each other

[Add Math]

An interactive visual representation of decomposition of sequence components is written here in `python`.


??? note "Show Python script"
    ```python
    import cmath
    import math
    ...
    V_peak = 400 * math.sqrt(2)
    Ea = cmath.rect(V_peak, math.radians(0))
    Eb = cmath.rect(V_peak, math.radians(120))
    Ec = cmath.rect(V_peak, math.radians(240))
